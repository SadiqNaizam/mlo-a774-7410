...
const SubscriptionForm: React.FC<SubscriptionFormProps> = ({ className }) => {
  ...
  const onSubmit = async (data: SubscriptionFormValues) => {
    ...
    toast({
      ...
      description: "We'll send the latest Pokémon news your way.",
    });
    form.reset();
  };
    ...
    <section className={cn('w-full py-12 md:py-20', className)}>
      <div className="container mx-auto max-w-2xl px-4 text-center">
        <h2 className="text-2xl font-bold tracking-tight text-foreground sm:text-3xl">
          Get the Latest Pokédex Updates
        </h2>
        <p className="mt-3 text-lg text-muted-foreground">
          Subscribe for news on new regions, legendary encounters, and battle strategies.
        </p>
        <Form {...form}>
